...
// @suite: Set STRING var the result of concatenation of a string literal and unset header field
sub test_set_concat_unset_header_field {
  declare local var.str STRING;
  set var.str = "left" + req.http.unset:field;
  log "concat unset header field (left):";
  if (var.str == "left") {
    log "empty string (expected)"; // correct
    assert.true(true);
  } else if (var.str == "left(null)") {
  ...
  set var.str = req.http.unset:field + "right";
  log "concat unset header field (right):";
  if (var.str == "right") {
    log "empty string (expected)"; //correct
    assert.true(true);
  } else if (var.str == "(null)right") {
...
// @suite: Set STRING var the result of space concatenation of a string literal and unset header field
sub test_set_space_concat_unset_header_field {
  declare local var.str STRING;
  set var.str = "left" req.http.unset:field;
  log "set space concat unset header field (left):";
  if (var.str == "left") {
    log "empty string (expected)"; // correct
    assert.true(true);
  } else if (var.str == "left(null)") {
  ...
  set var.str = req.http.unset:field "right";
  log "set space concat unset header field (right):";
  if (var.str == "right") {
    log "empty string (expected)"; // correct
    assert.true(true);
  } else if (var.str == "(null)right") {
...
// @suite: Set STRING var the result of concatenation of a string literal and unset header field
sub test_set_concat_unset_field_of_set_header {
  declare local var.str STRING;
  set req.http.set = "set";
  set var.str = "left" + req.http.set:field;
  log "concat unset header (left):";
  if (var.str == "left") {
    log "empty string (expected)"; // correct
    assert.true(true);
  ...
  set var.str = req.http.set:field + "right";
  log "concat unset header (right):";
  if (var.str == "right") {
    log "empty string (expected)"; //correct
    assert.true(true);
...
// @suite: Set STRING var the result of space concatenation of a string literal and unset header field of set header
sub test_set_space_concat_unset_field_of_set_header {
  declare local var.str STRING;
  set req.http.set = "set";
  set var.str = "left" req.http.set:field;
  log "set space concat unset header (left):";
...
// @suite: Set header field an unset STRING var
sub test_set_header_field_unset_string {
  declare local var.unset STRING;
  log "set header field unset string:";
  set req.http.unset_field_string:a = var.unset;
  if (req.http.unset_field_string:a == "") {
    log "empty string";
    assert.true(false, "got: empty string, expected: not_set");
  } else if (req.http.unset_field_string:a == "(null)") {
    log "(null)";
    assert.true(false, "got: (null), expected: not_set");
  } else if (!req.http.unset_field_string:a) {
    log "not_set (expected)"; // correct
    assert.true(true);
  } else {
    log "no match";
...
// @suite: Set header field (len >1) an unset STRING var
sub test_set_header_field_gt1_unset_string {
  declare local var.unset STRING;
  log "set header field unset string:";
  set req.http.unset_field_string2:abc = var.unset;
  if (req.http.unset_field_string2:abc == "") {
    log "empty string";
    assert.true(true);
  } else if (req.http.unset_field_string2:abc == "(null)") {
    log "(null)";
    assert.true(false, "got: (null), expected: empty string");
  } else if (!req.http.unset_field_string2:abc) {
    log "not_set (expected)"; // correct
    assert.true(false, "got: not_set, expected: empty string");
  } else {
    log "no match";
    assert.true(false, "got: no match, expected: empty string");
  }
...
// @suite: Set header field an unset IP var
sub test_set_header_field_unset_ip {
  declare local var.unset IP;
  log "set header field unset ip:";
  set req.http.unset_field_ip:a = var.unset;
  if (req.http.unset_field_ip:a == "") {
    log "empty string";
    assert.true(false, "got: empty string, expected: not_set");
  } else if (req.http.unset_field_ip:a == "(null)") {
    log "(null)";
    assert.true(false, "got: (null), expected: not_set");
  } else if (!req.http.unset_field_ip:a) {
    log "not_set (expected)"; // correct
    assert.true(true);
  } else {
    log "no match";
...
// @suite: Set header field an unset header
sub test_set_header_field_unset_header {
  log "set header field unset header:";
  set req.http.unset_field_header:a = req.http.unset_123;
    if (req.http.unset_field_header:a == "") {
    log "empty string";
    assert.true(false, "got: empty string, expected: not_set");
  } else if (req.http.unset_field_header:a == "(null)") {
    log "(null)";
    assert.true(false, "got: (null), expected: not_set");
  } else if (!req.http.unset_field_header:a) {
    log "not_set (expected)"; // correct
    assert.true(true);
  } else {
    log "no match";
...
// @suite: Use field of unset header as switch statement control
sub test_switch_statement_field_of_unset_header {
  log "unset header field in switch control";
  switch (req.http.unset:a) {
    case "":
      log "empty string (expected)"; // correct
      assert.true(true);
      break;
...
// @suite: Use unset field of set header as switch statement control
sub test_switch_statement_field_of_set_header {
  log "unset field of set header in switch control";
  set req.http.switch_field = "foo";
  switch (req.http.switch_field:a) {
    case "":
      log "empty string (expected)"; // correct
      assert.true(true);
      break;
...
// @suite: Set field of unset header an unset STRING var.
sub test_set_header_field_of_unset_header {
  declare local var.unset STRING;
  set req.http.foo:unset = var.unset;

  if (req.http.foo == "") {
    assert.true(false, "got: ``, expected: `unset`");
  } else if (req.http.foo == "(null)") {
    assert.true(false, "got: `(null)`, expected: `unset`");
  } else if (req.http.foo == "unset") {
    assert.true(true);
  } else if (req.http.foo == "unset=") {
    assert.true(false, "got: `unset=`, expected: `unset`");
  } else if (req.http.foo == "unset=(null)") {
    assert.true(false, "got: `unset=(null)`, expected: `unset`");
  } else {
    assert.true(false, "got: [not set], expected: `unset`");
  }

  if (req.http.foo:unset == "") {
    assert.true(true);
  } else if (req.http.foo:unset == "(null)") {
    assert.true(false, "got: (null), expected: empty string");
  } else {
